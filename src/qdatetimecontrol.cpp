...
#include "qdatetimecontrol.h"
#include <math.h>
#include <qdebug.h>
#include <qlocale.h>

QDateTimeControl::QDateTimeControl(QObject *parent)
    : QObject(new QDateTimeControlPrivate, parent)
{
    Q_D(QDateTimeControl);
    d->init(QDateTime(QDATETIMECONTROL_DATE_INITIAL, QDATETIMECONTROL_TIME_MIN));
}
...
QDateTimeControl::QDateTimeControl(const QVariant &variant, QVariant::Type parserType, QObject *parent)
    : QObject(*new QDateTimeControlPrivate, parent)
{
    Q_D(QDateTimeControl);
...
/*!
  \property QDateTimeControl::dateTime
  \brief the QDateTime that is set in the QDateTimeControl

  When setting this property the timespec of the QDateTimeControl remains the same
...
QDateTime QDateTimeControl::dateTime() const
{
    Q_D(const QDateTimeControl);
    return d->value.toDateTime();
}

void QDateTimeControl::setDateTime(const QDateTime &datetime)
{
    Q_D(QDateTimeControl);
    if (datetime.isValid()) {
        d->clearCache();
...
/*!
  \property QDateTimeControl::minimumDateTime
...
QDateTime QDateTimeControl::minimumDateTime() const
{
    Q_D(const QDateTimeControl);
    return d->minimum.toDateTime();
}

void QDateTimeControl::clearMinimumDateTime()
{
    setMinimumDateTime(QDateTime(QDATETIMECONTROL_DATE_MIN, QDATETIMECONTROL_TIME_MIN));
}

void QDateTimeControl::setMinimumDateTime(const QDateTime &dateTime)
{
    Q_D(QDateTimeControl);
    if (dateTime.isValid() && dateTime.date() >= QDATETIMECONTROL_DATE_MIN) {
...
QDateTime QDateTimeControl::maximumDateTime() const
{
    Q_D(const QDateTimeControl);
    return d->maximum.toDateTime();
}

void QDateTimeControl::clearMaximumDateTime()
{
    setMaximumDateTime(QDATETIMECONTROL_DATETIME_MAX);
}

void QDateTimeControl::setMaximumDateTime(const QDateTime &dateTime)
{
    Q_D(QDateTimeControl);
    if (dateTime.isValid() && dateTime.date() <= QDATETIMECONTROL_DATE_MAX) {
...
void QDateTimeControl::setDateTimeRange(const QDateTime &min, const QDateTime &max)
{
    Q_D(QDateTimeControl);
...
/*!
  \property QDateTimeControl::displayFormat
...
QString QDateTimeControl::displayFormat() const
{
    Q_D(const QDateTimeControl);
...
void QDateTimeControl::setDisplayFormat(const QString &format)
{
    Q_D(QDateTimeControl);
    if (d->parseFormat(format)) {
        ...
        if (timeShown && !dateShown) {
            ...
            setTimeRange(QDATETIMECONTROL_TIME_MIN, QDATETIMECONTROL_TIME_MAX);
...
/*!
    \property QDateTimeControl::timeSpec
    \brief the current timespec used by the date time edit.
*/

Qt::TimeSpec QDateTimeControl::timeSpec() const
{
    Q_D(const QDateTimeControl);
    return d->spec;
}

void QDateTimeControl::setTimeSpec(Qt::TimeSpec spec)
{
    Q_D(QDateTimeControl);
    if (spec != d->spec) {
        d->spec = spec;
...
QValidator::State QDateTimeControl::validate(QString &text, int &pos) const
{
    Q_D(const QDateTimeControl);
...
// --- QDateTimeControlPrivate ---

/*!
  \internal
  Constructs a QDateTimeControlPrivate object
*/


QDateTimeControlPrivate::QDateTimeControlPrivate()
    : QDateTimeParser(QVariant::DateTime, QDateTimeParser::DateTimeControl)
{
    ...
    minimum = QDATETIMECONTROL_COMPAT_DATETIME_MIN;
    maximum = QDATETIMECONTROL_DATETIME_MAX;

    readLocaleSettings();
}

void QDateTimeControlPrivate::updateTimeSpec()
{
...
void QDateTimeControlPrivate::updateCache(const QVariant &val, const QString &str) const
{
...
QDateTime QDateTimeControlPrivate::validateAndInterpret(QString &input, int &position,
...
/*!
  ...
  Internal function called by QDateTimeControl::stepBy(). Also takes a
  ...
  QDateTimeControl::stepEnabled() as well as QDateTimeControl::stepBy().
*/

QDateTime QDateTimeControlPrivate::stepBy(int sectionIndex, int steps, bool test) const
{
    Q_Q(const QDateTimeControl);
...
void QDateTimeControlPrivate::emitSignals(EmitPolicy ep, const QVariant &old)
{
    Q_Q(QDateTimeControl);
    if (ep == NeverEmit) {
        return;
    }
    pendingEmit = false;
...
*/
void QDateTimeControlPrivate::readLocaleSettings()
{
    const QLocale loc;
...
QString QDateTimeControlPrivate::getAmPmText(AmPm ap, Case cs) const
{
    if (ap == AmText) {
...
void QDateTimeControlPrivate::interpret(EmitPolicy ep)
{
    Q_Q(QDateTimeControl);
...
void QDateTimeControlPrivate::init(const QVariant &variant)
{
    Q_Q(QDateTimeControl);
    switch (variant.type()) {
    case QVariant::Date:
        value = QDateTime(variant.toDate(), QDATETIMECONTROL_TIME_MIN);
    ...
    case QVariant::DateTime:
        ...
        break;
    case QVariant::Time:
        value = QDateTime(QDATETIMECONTROL_DATE_INITIAL, variant.toTime());
        q->setDisplayFormat(defaultTimeFormat);
        if (sectionNodes.isEmpty()) // ### safeguard for broken locale
            q->setDisplayFormat(QLatin1String("hh:mm:ss"));
        break;
    default:
        Q_ASSERT_X(0, "QDateTimeControlPrivate::init", "Internal error");
        break;
    }
    updateTimeSpec();
}

QVariant QDateTimeControlPrivate::getZeroVariant() const
{
    Q_ASSERT(type == QVariant::DateTime);
    return QDateTime(QDATETIMECONTROL_DATE_INITIAL, QTime(), spec);
}

#include "moc_qdatetimecontrol.cpp"
