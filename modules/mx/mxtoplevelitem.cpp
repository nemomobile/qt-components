...
#include "mxtoplevelitem_p.h"

#include <QGraphicsScene>

MxTopLevelItemPrivate::MxTopLevelItemPrivate() :
...
void MxTopLevelItemPrivate::clearDependencyList()
{
    Q_Q(MxTopLevelItem);
...
*/
void MxTopLevelItemPrivate::initDependencyList()
{
    Q_Q(MxTopLevelItem);
...
void MxTopLevelItemPrivate::scheduleUpdateTransform()
{
    if (transformDirty)
        return;
...
void MxTopLevelItemPrivate::updateTransform()
{
    Q_ASSERT(targetItem);
    Q_Q(MxTopLevelItem);
...
void MxTopLevelItemPrivate::updateOpacity()
{
    Q_ASSERT(targetItem);
    Q_Q(MxTopLevelItem);
...
void MxTopLevelItemPrivate::updateVisible()
{
    Q_ASSERT(targetItem);
    Q_Q(MxTopLevelItem);
...
void MxTopLevelItemPrivate::updateParent()
{
    Q_ASSERT(targetItem);
    clearDependencyList();
    initDependencyList();
...
void MxTopLevelItemPrivate::updateWidthFromTarget()
{
    Q_ASSERT(targetItem);
    Q_Q(MxTopLevelItem);
    ...
    if (screenToParentTransform.isRotating() || itemToParentTransform.isRotating()) {
        qWarning() << "MxTopLevelItem: KeepInside feature is not supported together with rotation transforms";
        q->setX(newX);
        q->setWidth(newWidth);
        return;
    }
...
void MxTopLevelItemPrivate::updateHeightFromTarget()
{
    Q_ASSERT(targetItem);
    Q_Q(MxTopLevelItem);
...
void MxTopLevelItemPrivate::setZFromSiblings()
{
    Q_Q(MxTopLevelItem);
    int maxZ = 0;
    ...
    for (int i = siblings.count() - 1; i >= 0; --i) {
        ...
        if (qobject_cast<MxTopLevelItem *>(obj))
            continue;
...
MxTopLevelItem::MxTopLevelItem(QDeclarativeItem *parent) :
    QDeclarativeItem(parent), d_ptr(new MxTopLevelItemPrivate)
{
    d_ptr->q_ptr = this;
}

MxTopLevelItem::MxTopLevelItem(MxTopLevelItemPrivate &dd, QDeclarativeItem *parent) :
    QDeclarativeItem(parent), d_ptr(&dd)
{
    d_ptr->q_ptr = this;
}

MxTopLevelItem::~MxTopLevelItem()
{

}

bool MxTopLevelItem::keepInside() const
{
    Q_D(const MxTopLevelItem);
    return d->keepInside;
}

void MxTopLevelItem::setKeepInside(bool keepInside)
{
    Q_D(MxTopLevelItem);
...
QVariant MxTopLevelItem::itemChange(GraphicsItemChange change, const QVariant &value)
{
    Q_D(MxTopLevelItem);
